<template>
	<aside class="sidebar" :class="{'open':isSidebar}">
		<div @click="toggle('isSidebar')">
			<button type="button" class="tcon tcon-menu--arrow tcon-menu--arrowleft" :class="{'tcon-transform':isSidebar}" >
...
export default {
  ...
  data () {
	...
	computed: {
  		isSidebar(){
			return this.$store.state.sidebar.openSidebar
		}
	},
	methods:{
	toggle(param){
				...
				a{
					...
					position: relative;
						...
						&::before {
							...
							transition: all .2s cubic-bezier(0, -0.22, 0.18, 1.06);
